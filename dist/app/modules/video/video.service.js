"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.videoService = void 0;
const http_status_codes_1 = require("http-status-codes");
const AppError_1 = __importDefault(require("../../errorHelpers/AppError"));
const video_model_1 = require("./video.model");
const createVideo = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    return video_model_1.Video.create(payload);
});
const getAllVideos = () => __awaiter(void 0, void 0, void 0, function* () {
    return video_model_1.Video.find({ isActive: true }).sort({ createdAt: -1 });
});
const getAllVideosAdmin = () => __awaiter(void 0, void 0, void 0, function* () {
    return video_model_1.Video.find().sort({ createdAt: -1 });
});
const updateVideo = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const video = yield video_model_1.Video.findById(id);
    if (!video)
        throw new AppError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'Video not found');
    return video_model_1.Video.findByIdAndUpdate(id, payload, { new: true });
});
const deleteVideo = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const video = yield video_model_1.Video.findById(id);
    if (!video)
        throw new AppError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'Video not found');
    return video_model_1.Video.findByIdAndDelete(id);
});
exports.videoService = { createVideo, getAllVideos, getAllVideosAdmin, updateVideo, deleteVideo };
